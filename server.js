...
console.log('--- MBTI 分析器启动 ---');
console.log('接入点 ID:', MODEL_ID);
console.log('API Key:', ARK_API_KEY ? '已加载' : '未找到');

app.post('/api/analyze', upload.any(), async (req, res) => {
    try {
        console.log('\n[Analyze] 收到分析请求, 图片数量:', req.files ? req.files.length : 0);
        ...
        for (const url of REGIONS) {
            try {
                ...
                if (status !== 404) throw err; // 如果不是 404，说明路径对但其他地方错了，直接抛出
            }
        }
        ...
        try {
            ...
            res.status(500).json({ error: 'AI 返回内容格式解析失败', raw: content });
        }

    } catch (error) {
        const errorData = error.response ? error.response.data : error.message;
        const statusCode = error.response ? error.response.status : 500;
        console.error(`[Error] 分析失败 (状态码 ${statusCode}):`, JSON.stringify(errorData, null, 2));
...
app.listen(port, () => {
    console.log(`服务已在 http://localhost:${port} 运行`);
});
